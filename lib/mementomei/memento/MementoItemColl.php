 ...
 * @author caiofior
 */
class MementoItemColl extends \ContentColl {
      public function __construct($db) {
        parent::__construct(new \mementomei\memento\MementoItem($db));           
      }
      /**
    ...
    protected function setFilter ($select,$criteria) {
       $select->join('memento', 'memento.code=beloved_memento.memento_code', array('name'), \Zend\Db\Sql\Select::JOIN_LEFT);
       if (array_key_exists('sSearch', $criteria) && $criteria['sSearch'] != '') {
          $select->where('`data` LIKE "%'.addslashes($criteria['sSearch']).'%"');
       }
      return $select;
    }
}